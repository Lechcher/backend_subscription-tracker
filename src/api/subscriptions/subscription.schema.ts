...
export const subscriptionSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    const normalizedRenewalDate = new Date(
      data.renewalDate.setHours(0, 0, 0, 0)
    );

    // Business rule: Renewal must be at least one day after start
    if (normalizedRenewalDate <= normalizedStartDate) {
      ctx.addIssue({
        code: "custom",
        message: "Renewal date must be at least one day after the start date",
        path: ["renewalDate"],
      });
    }

    // Business rule: Prevent excessively far future renewal dates
    const maxFutureDate = new Date();
    maxFutureDate.setFullYear(maxFutureDate.getFullYear() + 10); // 10-year maximum

    if (normalizedRenewalDate > maxFutureDate) {
      ctx.addIssue({
        code: "custom",
        message: "Renewal date cannot be more than 10 years in the future",
        path: ["renewalDate"],
      });
    }
  });
